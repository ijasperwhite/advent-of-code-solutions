...
class DayEight {
                ...
                .map { rowIndex ->
                    (columnList.indices).toList().map { columnIndex ->
                        ...
                        val leftScore =
                                getViewLength(
                                        currentValue,
                                        rowList[rowIndex].subList(0, columnIndex).reversed()
                                )
                        val rightScore =
                                getViewLength(
                                        currentValue,
                                        rowList[rowIndex].subList(columnIndex + 1, rowList.size)
                                )
                        val topScore =
                                getViewLength(
                                        currentValue,
                                        columnList[columnIndex].subList(0, rowIndex).reversed()
                                )
                        val bottomScore =
                                getViewLength(
                                        currentValue,
                                        columnList[columnIndex].subList(
                                                rowIndex + 1,
                                                columnList.size
                                        )
                                )
    ...
    private fun List<String>.toColumnsList(): List<List<Int>> {
        ...
        val new =
                (0 until this.size).map { rowIndex ->
                    (0 until initialList[0].size).toList().map { columnIndex ->
                        initialList[columnIndex][rowIndex]
                    }
                }
