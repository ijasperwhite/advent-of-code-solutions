...
class DayTwelve {
    fun partOne(list: List<String>): Long {
        ...
        val results =
                (0..100).toList().mapIndexedNotNull { index, _ ->
                    println("")
                    ...
                    (0..26).toList().map {
                        val subResult =
                                findNextSubPath(current, current, coordinates, visited, invalids)
    ...
    data class Coordinate(val x: Long, val y: Long, val z: Long, val char: Char)

    private fun toCoordinates(l: List<String>): List<Coordinate> {
        return l
                .mapIndexed { rowIndex, r ->
